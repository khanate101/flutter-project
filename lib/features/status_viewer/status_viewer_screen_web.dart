...
class _StatusViewerScreenState extends ConsumerState<StatusViewerScreen> {
  ...
  void initState() {
    ...
    if (widget.item.isVideo) {
      _controller = VideoPlayerController.networkUrl(Uri.parse(widget.item.uri))
  ...
  Future<void> _save() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(saved ? 'تم تنزيل الملف ✓' : 'تعذر تنزيل الملف')));
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(onPressed: _share, icon: const Icon(Icons.download)),
          IconButton(onPressed: _busy ? null : _save, icon: _busy ? const CircularProgressIndicator() : const Icon(Icons.save_alt)),
        ],
      ),
  ...
  Widget _video() {
    ...
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ConstrainedBox(constraints: const BoxConstraints(maxWidth: 1000, maxHeight: 650), child: AspectRatio(aspectRatio: c.value.aspectRatio, child: VideoPlayer(c))),
